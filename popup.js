...
function card(item) { const el = document.createElement("article"); el.className = "item"; el.innerHTML = `<div class="label"></div><div class="url"></div><button class="copy">Copy</button><button class="open">Open in new tab</button>`; el.querySelector(".label").textContent = `${activeTabTitle} · ${item.language || "Unknown"}`; el.querySelector(".url").textContent = item.url; el.querySelector(".copy").onclick = () => navigator.clipboard.writeText(item.url).then(() => $("status").textContent = "URL copied."); el.querySelector(".open").onclick = () => chrome.tabs.create({ url: item.url }); return el; }
